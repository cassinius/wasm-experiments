...
impl Universe {
	...
	pub fn toggle_cell(&mut self, row: u32, col: u32) {
		...
		self.cells_l.set(idx_l, !self.cells_l[idx_l]);

		let idx_p = self.get_index_p(row+1, col +1);
		self.cells_p.set(idx_p, !self.cells_p[idx_p]);
	}

	pub fn reset_cells(&mut self) {
		// logical
		let size_l = (self.width_l * self.height_l) as usize;
		for idx_l in 0..size_l { self.cells_l.set(idx_l, false) }
		// physical
		let size_p = (self.width_p * self.height_p) as usize;
		for idx_p in 0..size_p { self.cells_p.set(idx_p, false) }
	...
	pub fn randomize_cells(&mut self) {
		for row in 0..self.height_l {
			for col in 0..self.width_l {
				...
				self.cells_l.set(idx_l, val);
				self.cells_p.set(idx_p, val);
			}
		}
	}
